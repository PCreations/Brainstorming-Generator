		...
		$.ajax({
			...
			success: function(data) {
				...
				if (!_this.wordExists(json)) {
					...
					console.log("string apres clean = "+string+" length = "+string.length);
					if (string.length > 0) _this.selectedWords = string.split(",");
					else _this.selectedWords = new Array();	
				}
				else {
					...
					if (json['data']['1']['ss_type'] != 'n') {
						...
						string = _this.strCleaner(",", _this.wordsChecker[0], string);
						if (string.length > 0) _this.selectedWords = string.split(",");
						else _this.selectedWords = new Array(); //si aucun mot alors on efface le tableau pour ne pas envoyer de mots vides
					}
				}
			...
			complete: function(xhr, msg) {
				if (_this.wordsChecker.length > 0) {
					console.log("wordChecker.length = "+_this.wordsChecker.length);
					_this.wordsCheck(callback, BSref);
				}
				else {
					console.log("MOT TRIES : ");
